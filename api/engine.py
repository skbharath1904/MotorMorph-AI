...
def generate_motor_design_logic(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if is_cv:
        return calculate_cv_no_error_guarantee(inputs, notes)
...
def calculate_cv_no_error_guarantee(inputs: Dict[str, Any], notes: list) -> Dict[str, Any]:
    ...
    if t_motor > 2000:
        ...
        if gear_ratio > 14:
            gear_ratio = 14
            t_motor = t_wheel / gear_ratio
            notes.append("High tractive force exceeds motor torque limit at 14:1 gear ratio. Performance limited.")
    elif t_motor < 600:
        t_motor = 600
        gear_ratio = t_wheel / t_motor
        if gear_ratio < 8:
    ...
    if peak_power_kw > 350:
        ...
        notes.append("Peak power hit 350kW limit. Base speed reduced to maintain torque consistency.")
    ...
    reason = "SRM: Selected for high torque, ruggedness, and low cost in heavy commercial trucks."
    if total_mass < 8000:
    ...
    power_density = 3.8 if 'PMSM' in motor_type else 2.8 if 'IM' in motor_type else 2.0
    weight = peak_power_kw / power_density

    torque_density = 38 if 'PMSM' in motor_type else 28 if 'IM' in motor_type else 20
    volume_l = t_motor / torque_density
    volume_m3 = volume_l / 1000

    st_od_m = ((4 * volume_m3) / math.pi) ** (1/3)
    len_m = st_od_m
    if len_m / st_od_m < 0.6:
        len_m = st_od_m * 0.6
        st_od_m = math.sqrt((4 * volume_m3) / (math.pi * len_m))
    if len_m / st_od_m > 1.4:
        len_m = st_od_m * 1.4
        st_od_m = math.sqrt((4 * volume_m3) / (math.pi * len_m))
    ...
    if phase_current > 600:
        ...
        if v_system > 800:
            ...
            t_motor = (peak_power_kw * 1000 * 60) / (2 * math.pi * base_rpm)
            notes.append("Electrical limit hit. Peak power and torque reduced to stay within 600A current limit.")

    total_losses_kw = peak_power_kw * (1 - op_eff)
    copper_loss_kw = total_losses_kw * 0.45
    iron_loss_kw = total_losses_kw * 0.35
    switching_loss_kw = total_losses_kw * 0.20

    # Validation Resync
    val_p = (t_motor * 2 * math.pi * base_rpm) / (60 * 1000)
    if abs(val_p - peak_power_kw) > 1.0:
        peak_power_kw = val_p

    return format_output_cv({
        'motorType': motor_type, 'reason': reason, 'notes': notes, 'peakPowerKw': peak_power_kw, 
        'continuousPowerKw': continuous_power_kw, 'tMotor': t_motor, 'motorRpm': motor_rpm, 
        'baseRpm': base_rpm, 'v_system': v_system, 'weight': weight, 'statorOd': stator_od, 
        'length': length, 'opEff': op_eff, 'phaseCurrent': phase_current, 'gearRatio': gear_ratio, 
        'totalMass': total_mass, 'wheelRadius': wheel_radius, 'wheelRpm': wheel_rpm, 
        'copperLossKw': copper_loss_kw, 'ironLossKw': iron_loss_kw, 'switchingLossKw': switching_loss_kw
    })

def format_output_cv(d: Dict[str, Any]) -> Dict[str, Any]:
    peak_eff = round(d['opEff'] * 100 + 1.2, 1)
    air_gap = 0.8 if d['statorOd'] < 350 else 1.5
    rotor_mass = d['weight'] * 0.35
    rotor_rad_m = (d['statorOd'] * 0.62) / 2000
    ...
    for r in range(0, int(d['motorRpm']) + 1000, 500):
        ...
        eff_curve = (d['opEff'] * 100) * (1 - ((r / d['motorRpm'] - 0.65) ** 2) * 0.22) if r > 0 else 0
        eff_curve = max(0, min(float(peak_eff), eff_curve))
        curve.append({'rpm': r, 'torque': round(t_curve), 'efficiency': round(eff_curve, 1)})

    p_range = 230 # 350 - 120
    st_res = 0.003 + (0.03 - 0.003) * (1 - (d['peakPowerKw'] - 120)/p_range)
    dq_ind = 0.5 + (8.0 - 0.5) * (1 - (d['peakPowerKw'] - 120)/p_range)
    ...
    return {
        ...
        'thermal': {
            'coolingMethod': "Liquid + Oil Cooling" if d['peakPowerKw'] > 250 else "Liquid Cooling",
            'maxCoilTemp': "150°C",
            'coolantFlowRate': f"{round(d['peakPowerKw'] * 0.07, 1)} L/min",
            'thermalResistance': "0.420 K/W",
            'lossBreakdown': f"Copper: {d['copperLossKw']:.1f}kW | Iron: {d['ironLossKw']:.1f}kW | Switching: {d['switchingLossKw']:.1f}kW"
        },
        'dimensions': {
        ...
        'electrical': {
            ...
            'backEmfConstant': f"{round(d['v_system'] * 0.82 / (2 * math.pi * d['motorRpm'] / 60), 4) if d['motorRpm'] > 0 else 0.1:.4f} V·s/rad",
        ...
        'mechanical': {
            ...
            'maxCentrifugalForce': f"{round(d['weight'] * 110)} N",
            'bearingLoad': f"{round(d['weight'] * 18 + d['tMotor'] * 2.2)} N",
            'coggingTorque': f"{round(d['tMotor'] * 0.014, 1)} Nm",
...
def format_output(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'mechanical': {
            ...
            'vehicleMass': f"{round(d['total_mass'])} kg",
            'totalVehicleMass': f"{round(d['total_mass'])} kg",
            'gearRatio': f"{round(d['gearRatio'], 2)}:1",
            'wheelTorque': f"{round(d['tMotor'] * d['gearRatio'])} Nm"
        },
        'performanceCurve': curve
